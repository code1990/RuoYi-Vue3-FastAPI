...
class StockDdeDao:
    ...
    @staticmethod
    def get_combo_statistics(database_path: str, start_date: str | None, end_date: str | None, target_return_pct: float) -> list[dict]:
        path = Path(database_path)
        if not path.is_file():
            raise FileNotFoundError(f'Stock statistics database does not exist: {path}')
        ...
        with sqlite3.connect(uri, uri=True) as connection:
            ...
            rows = connection.execute(
                f'''SELECT signal_date,
                           CASE WHEN previous_signal_count + today_signal_count = 2 THEN '2次'
                                WHEN previous_signal_count + today_signal_count = 3 THEN '3次'
                                ELSE '4次+' END AS combo_band,
                    ...
                    FROM t_stock_dde_combo_signal
                    WHERE {where_sql}
                    GROUP BY signal_date, combo_band
                    ORDER BY signal_date, CASE combo_band WHEN '2次' THEN 1 WHEN '3次' THEN 2 ELSE 3 END''',
                params,
            ).fetchall()
