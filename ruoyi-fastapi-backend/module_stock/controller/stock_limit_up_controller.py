...
@stock_limit_up_controller.get('/theme/top15', summary='查询涨停题材历史前15', response_model=DataResponseModel[StockLimitUpThemeListModel])
async def get_stock_limit_up_theme_top15(
    start_date: Annotated[str | None, Query(alias='startDate', pattern=r'^\d{4}-\d{2}-\d{2}$')] = None,
    end_date: Annotated[str | None, Query(alias='endDate', pattern=r'^\d{4}-\d{2}-\d{2}$')] = None,
) -> Response:
    try:
        result = await StockLimitUpService.get_theme_top15_services(start_date, end_date)
