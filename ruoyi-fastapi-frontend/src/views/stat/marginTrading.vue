<template><div class="stat-page"><el-alert :title="`${windowDays || '单日'}融资列表：按累计评分分组；完整5日样本中，任一天最高收益≥1.8%为达标。`" type="info" :closable="false" show-icon /><el-card :header="`${windowDays || '单日'}融资累计评分与5日结果`" class="chart-row"><div ref="chartRef" v-loading="loading" class="chart" /></el-card></div></template>
<script setup>
...
function renderChart(rows) {
  const dates = [...new Set(rows.map(row => row.signalDate))].sort().slice(-30); const values = new Map(rows.map(row => [`${row.signalDate}:${row.scoreBand}`, row]))
