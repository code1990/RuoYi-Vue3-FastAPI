  ...
  <div class="stock-data-page">
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" border>
        ...
        <el-table-column label="策略" min-width="100"><template #default="{ row }">{{ strategyName(row.strategy) }}</template></el-table-column>
        <el-table-column label="信号" min-width="80"><template #default="{ row }">{{ signalName(row.signalType) }}</template></el-table-column>
...
const total = ref(0)
const dateRange = ref([])
const query = reactive({ pageNum: 1, pageSize: 20, stockCode: '' })

function getList() {
  loading.value = true
  listMainFundPerformance({ ...query, stockCode: query.stockCode || undefined, startDate: dateRange.value?.[0], endDate: dateRange.value?.[1] }).then(response => {
...
function strategyName(value) {
  return { '55d_main_fund_v1': '55日主力资金' }[value] || value || '-'
}

function signalName(value) {
  return { accumulation: '资金蓄势' }[value] || value || '-'
}
