  ...
  <div class="dde-observation-page">
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" border @sort-change="handleSortChange">
        <el-table-column label="交易日" prop="tradeDate" sortable="custom" width="95" /><el-table-column label="股票" min-width="110"><template #default="{ row }">{{ row.stockCode }} {{ row.stockName }}</template></el-table-column><el-table-column label="DDE次数" prop="signalCount" sortable="custom" width="80" /><el-table-column label="早/午/尾" min-width="80"><template #default="{ row }">{{ row.morningCount }}/{{ row.noonCount }}/{{ row.closeCount }}</template></el-table-column><el-table-column label="最佳排名" prop="bestRank" sortable="custom" width="85" />
        <el-table-column v-if="dimension === 'intraday_combo'" label="日内连续" min-width="120"><template #default="{ row }">{{ comboName(row.comboType) }}</template></el-table-column><el-table-column label="14:40价格" prop="entryPrice" sortable="custom" min-width="95"><template #default="{ row }">{{ number(row.entryPrice) }}</template></el-table-column><el-table-column label="市值" prop="marketCap" sortable="custom" min-width="85"><template #default="{ row }">{{ amount(row.marketCap) }}</template></el-table-column><el-table-column label="涨跌幅" prop="changePct" sortable="custom" min-width="80"><template #default="{ row }">{{ percent(row.changePct) }}</template></el-table-column>
