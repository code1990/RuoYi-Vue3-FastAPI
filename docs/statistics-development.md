...
- 路由：`/stat/main-fund`
- 接口：`GET /stock/main-fund/statistics?targetReturnPct=1.8`
...
融资融券使用参与度分组；融资2/3/5天使用各自组合列表的累计评分 `0-0.3`、`0.3-0.6`、`0.6+` 分组，避免将不同窗口的选股差异汇总掉。仅完整5日样本入图，五日内任一天最高收益达到目标收益为达标。
